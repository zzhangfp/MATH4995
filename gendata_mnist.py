...
def main() -> None:
    ...
    parser.add_argument(
        "--output_file", help="file for saving the data output (.gz file)", type=str, default="s3_mnist.gz", required=False
    )
    ...
    for label, data in zip(["train", "test"], [mnist_train, mnist_test]):
        ...
        current = 0
        signals = data["images"].reshape(-1, 28, 28).numpy().astype(np.float64)
