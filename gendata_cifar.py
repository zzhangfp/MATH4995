#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2024/11/24 20:46
# @Author  : CodeCat
# @File    : gendata_c.py
# @Software: PyCharm
#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2024/11/24 19:49
# @Author  : CodeCat
# @File    : gendata.py
# @Software: PyCharm
...
def main() -> None:
    ...
    parser.add_argument(
        "--mnist_data_folder", help="folder for saving the mnist data", type=str, default="data", required=False
    )
    parser.add_argument(
        "--output_file", help="file for saving the data output (.gz file)", type=str, default="s3_cifar.gz", required=False
    )
    ...
    trainset = datasets.CIFAR10(root=args.mnist_data_folder, train=True, download=True)
    testset = datasets.CIFAR10(root=args.mnist_data_folder, train=False, download=True)
    mnist_train = {}
    mnist_train["images"] = trainset.data
    mnist_train["labels"] = trainset.targets
    mnist_test = {}
    mnist_test["images"] = testset.data
    mnist_test["labels"] = testset.targets
    ...
    for label, data in zip(["train", "test"], [mnist_train, mnist_test]):
        ...
        current = 0
        signals = data["images"].reshape(-1, 3, 32, 32).astype(np.float64)
        n_signals = signals.shape[0]
        tmp = np.zeros((n_signals, 32, 32)).astype(np.float64)
        for i in range(n_signals):
            # print(signals[i].transpose(1, 2, 0).shape)
            img = Image.fromarray(np.uint8(signals[i].transpose(1, 2, 0))).convert("L")
            img = np.asarray(img).astype(np.float64)
            tmp[i] = img
        signals = tmp
        projections = np.ndarray((n_signals, 2 * args.bandwidth, 2 * args.bandwidth), dtype=np.uint8)
